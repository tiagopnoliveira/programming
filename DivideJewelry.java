  ...
  static int[] go(int pos, int cset, int csum) { 
    ...
    public static int[] divide(int[] x) {
		...
		if(res == null) {
			...
			Arrays.fill(initVisitedNodes, 0);
		}
		return res;
	}
	...
	private static int[] divideAux(Hashtable<Integer, Boolean[]> res, int[] x, Boolean[] visitedNodes, int pos, int currentSum, int remainingSum) {
		if(currentSum > remainingSum || pos >= x.length) {
			return null;
		}
		...
		finalRes = divideAux(res, x, newVisitedNodes, pos+1, newSum, remainingSum-x[pos]);
		if(finalRes == null) {
			return divideAux(res, x, visitedNodes, pos+1, currentSum, remainingSum);
		} else {
			return finalRes;
		}
	}
