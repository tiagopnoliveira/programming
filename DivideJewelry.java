...
//	private static int[] a = {42,4,6,10,6,4,5,7};
	...
	private static int[] killer = {17,7,389,811,1993,4177,35281,37277,824489,849383,457189,333479,228923,99793,473377,426331,2153,45337,157229,711317,441647,355093,863729,999959,429161};
  ...
  static int[] go(int pos, int cset, int csum) { 
    ...
    public static void main(String[] args) {
		...
		System.out.println("Result own: " + Arrays.toString(res));
		System.out.println();
    ...
    public static int[] divide(int[] x) {
		...
		// for adding (or not) the elements are exhausted with only 20 elements (2^n - 1). However, since the values are adding themselves, we can easily go
		// over a total sum of 1M, but since we are already limiting ourselves to 20 elements, the end result can be higher than 20 * (1M-20). For this purpose,
		// we just have to expand the potential permutations to 25 elements, as 2^25 - 1 [32MB] is smaller than 25 * (1M-25) [25M].
		// Checking only one permutation, we only get the each value (1:1). Checking 2 permutations, we have 3 possible values (10/01/11).
		// Checking 3 permutations, we have 7 possible values (100/010/001/110/101/011/111). Checking 25 permutations, we have 32MB - 1 possible values, thus
		// covering the entire range stated in the problem exercise (25 * 1M-25).
		n = Math.min(x.length, 25); 

		int totalSum = sumAllArray(x);
		Boolean[] initVisitedNodes = new Boolean[n];
		...
		// so a solution permutation must fit in 25 bits in any case. TopCoder solution also stores the sum collision table
		// in an array instead of a hashtable. This can be done with the caveat that we will require 25 x 1M slots
		// (as we can have a case of 1M+999999+999998+...+999976)
	...
	private static int[] divideAux(Hashtable<Integer, Boolean[]> res, int[] x, Boolean[] visitedNodes, int pos, int currentSum, int remainingSum) {
		...
		if(res.containsKey(newSum)) {
			finalRes = processMatchFound(res.get(newSum), newVisitedNodes, x.length);
			if(finalRes != null) {
				return finalRes;
			}
		}
	...
	private static int[] processMatchFound(Boolean[] visitedNodes1, Boolean[] visitedNodes2, int xLength) {
		perfCounter++;
